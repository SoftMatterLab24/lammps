...
PairPOLYSpring::PairPOLYSpring(LAMMPS *_lmp) : Pair(_lmp), k(nullptr), ka(nullptr), cut(nullptr), gamma(nullptr)
{
  writedata = 1;
  anharmonic_flag = 0;
}
...
void PairPOLYSpring::compute(int eflag, int vflag)
{
...
void PairPOLYSpring::settings(int narg, char ** arg)
{
  int iarg = 0;
  while (iarg < narg) {
    if (strcmp(arg[iarg], "anharmonic") == 0) {
      if (iarg + 1 >= narg)
        utils::missing_cmd_args(FLERR, "pair_coeff poly/spring anharmonic", error);
...
void PairPOLYSpring::init_style()
{
  if (comm->ghost_velocity == 0)
    error->all(FLERR, "Pair poly/spring requires ghost atoms store velocity");
...
double PairPOLYSpring::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
...
void PairPOLYSpring::write_restart(FILE *fp)
{
...
void PairPOLYSpring::read_restart(FILE *fp)
{
...
void PairPOLYSpring::write_restart_settings(FILE *fp)
{
...
void PairPOLYSpring::read_restart_settings(FILE *fp)
{
  if (comm->me == 0)
...
void PairPOLYSpring::write_data(FILE *fp)
{
  if (anharmonic_flag) {
...
void PairPOLYSpring::write_data_all(FILE *fp)
{
  if (anharmonic_flag) {
    for (int i = 1; i <= atom->ntypes; i++)
...
double PairPOLYSpring::single(int i, int j, int itype, int jtype, double rsq, double /*factor_coul*/,
