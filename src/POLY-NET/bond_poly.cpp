    ...
    fix_update_special_bonds(nullptr), pack_choice(nullptr), output_data(nullptr)
{
  ...
  id_fix_dummy_special = utils::strdup(fmt::format("POLY_DUMMY_SPECIAL_{}", instance_total));
  modify->add_fix(fmt::format("{} all DUMMY ", id_fix_dummy_special));

  id_fix_dummy_history = utils::strdup(fmt::format("POLY_DUMMY_HISTORY_{}", instance_total));
  ...
  if (!ignore_special_flag) {
    if (overlay_flag) {
      if (force->special_lj[1] != 1.0 || force->special_lj[2] != 1.0 ||
        ...
        error->all(FLERR,
                   "With overlay/pair yes, POLY bond styles require a value of 1.0 for all "
      ...
      if (force->newton_bond && break_flag)
        error->all(FLERR,
                   "With overlay/pair no, or break yes, POLY bond styles require Newton bond off");
      ...
      if (break_flag &&
        ...
        error->all(FLERR,
                   "With overlay/pair no, and break yes, POLY bond styles requires special Coulomb "
                   "weights = 1,1,1");
      ...
      if (id_fix_dummy_special && break_flag) {
        ...
        if (fixes.size() > 0 ) {
          ...
          id_fix_update_special_bonds = utils::strdup("POLY_UPDATE_SPECIAL_BONDS");
    ...
    if (force->special_lj[2] != 1.0 || force->special_lj[3] != 1.0 ||
        force->special_coul[2] != 1.0 || force->special_coul[3] != 1.0)
      error->all(FLERR, "Bond style poly requires 1-3 and 1-4 special weights of 1.0");
  }

  if (break_flag) {
    if (force->angle || force->dihedral || force->improper)
      error->all(FLERR, "Bond style poly cannot break with 3,4-body interactions");
    if (atom->molecular == 2)
      error->all(FLERR, "Bond style poly cannot break with atom style template");
  }
...
void BondPoly::settings(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "store/local") == 0) {
      ...
      if (iarg + 1 > narg)
        error->all(FLERR, "Illegal bond poly command, missing option for overlay/pair");
...
void BondPoly::pack_id1(int n, int i, int /*j*/)
{
  tagint *tag = atom->tag;
  output_data[n] = tag[i];
}
...
void BondPoly::pack_id2(int n, int /*i*/, int j)
{
  tagint *tag = atom->tag;
  output_data[n] = tag[j];
}
...
void BondPoly::pack_time(int n, int /*i*/, int /*j*/)
{
