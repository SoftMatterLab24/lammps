...
BondPolydFJC::BondPolydFJC(LAMMPS *_lmp) :
    BondPoly(_lmp), Ks(nullptr), Kj(nullptr), rcrit(nullptr), gamma(nullptr), zeta(nullptr),
...
double BondPolydFJC::store_bond(int n, int i, int j) // !! This is not updated for GKV !!
{
  int type;
...
void BondPolydFJC::compute(int eflag, int vflag)
{
...
void BondPolydFJC::coeff(int narg, char **arg)
{
...
void BondPolydFJC::init_style()
{
  BondPoly::init_style();

  if (comm->ghost_velocity == 0)
    error->all(FLERR, "Bond poly/dFJC requires ghost atoms store velocity");

}

/* ---------------------------------------------------------------------- */

void BondPolydFJC::settings(int narg, char **arg)
{
  ...
  for (std::size_t i = 0; i < leftover_iarg.size(); i++) {
    ...
    if (strcmp(arg[iarg], "smooth") == 0) {
      if (iarg + 1 > narg) error->all(FLERR, "Illegal bond poly/dFJC command, missing option for smooth");
      smooth_flag = utils::logical(FLERR, arg[iarg + 1], false, lmp);
      i += 1;
    } else if (strcmp(arg[iarg], "normalize") == 0) {
      if (iarg + 1 > narg) error->all(FLERR, "Illegal bond poly/dFJC command, missing option for normalize");
      normalize_flag = utils::logical(FLERR, arg[iarg + 1], false, lmp);
      i += 1;
    } else if (strcmp(arg[iarg], "temp/shift") == 0) {
      if (iarg + 1 > narg) error->all(FLERR, "Illegal bond poly/dFJC command, missing option for temp/shift");
      temperature_flag = utils::logical(FLERR, arg[iarg + 1], false, lmp);
      i += 1;
    } else {
      error->all(FLERR, "Illegal bond poly/dFJC command, invalid argument {}", arg[iarg]);
    }
  }
  ...
  if (smooth_flag && !break_flag)
    error->all(FLERR, "Illegal bond poly/dFJC command, must turn off smoothing with break no option");
...
void BondPolydFJC::write_restart(FILE *fp)
{
...
void BondPolydFJC::read_restart(FILE *fp)
{
...
void BondPolydFJC::write_restart_settings(FILE *fp)
{
...
void BondPolydFJC::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double BondPolydFJC::single(int type, double rsq, int i, int j, double &fforce)
{
...
void BondPolydFJC::null_table(Table *tb)
{
...
void BondPolydFJC::free_table(Table *tb)
{
...
void BondPolydFJC::read_table(Table *tb, char *file, char *keyword)
{
  double dt = update->dt;
...
void BondPolydFJC::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->r0 = 0.0;
...
/* ---------------------------------------------------------------------- */
 ...
 void BondPolydFJC::update_table(int type)
{   
  double dt = update->dt;
...
void BondPolydFJC::bond_lookup(int type, int i, int j, int &N, double &b)
{
  int iatom, jatom;
  const Table *tb = &tables[tabindex[type]];
...
void BondPolydFJC::bcast_table(Table *tb) // *UPDATED
{
...
void BondPolydFJC::direct_solve(double r, int type, int n, double dt, double &f)
{
...
void BondPolydFJC::iter_solve(double r, int type, int n, double dt, double &f)
{
  double N, b, eta;
  double fn, qn, yn;
...
void *BondPolydFJC::extract(const char *str, int &dim)
{
  dim = 1;
