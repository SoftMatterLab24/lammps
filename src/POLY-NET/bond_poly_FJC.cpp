...
BondPolyFJC::BondPolyFJC(LAMMPS *_lmp) :
    BondPoly(_lmp), k0(nullptr), fcrit(nullptr), gamma(nullptr), lamc(nullptr),
...
double BondPolyFJC::store_bond(int n, int i, int j)
{
  int type;
...
void BondPolyFJC::compute(int eflag, int vflag)
{
...
void BondPolyFJC::coeff(int narg, char **arg)
{
...
void BondPolyFJC::init_style()
{
  BondPoly::init_style();

  if (comm->ghost_velocity == 0)
    error->all(FLERR, "Bond poly/FJC requires ghost atoms store velocity");

}

/* ---------------------------------------------------------------------- */

void BondPolyFJC::settings(int narg, char **arg)
{
  ...
  for (std::size_t i = 0; i < leftover_iarg.size(); i++) {
    ...
    if (strcmp(arg[iarg], "smooth") == 0) {
      if (iarg + 1 > narg) error->all(FLERR, "Illegal bond poly/FJC command, missing option for smooth");
  ...
  if (smooth_flag && !break_flag)
    error->all(FLERR, "Illegal bond poly/FJC command, must turn off smoothing with break no option");
...
void BondPolyFJC::write_restart(FILE *fp)
{
...
void BondPolyFJC::read_restart(FILE *fp)
{
...
void BondPolyFJC::write_restart_settings(FILE *fp)
{
...
void BondPolyFJC::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double BondPolyFJC::single(int type, double rsq, int i, int j, double &fforce)
{
...
void BondPolyFJC::null_table(Table *tb)
{
...
void BondPolyFJC::free_table(Table *tb)
{
...
void BondPolyFJC::read_table(Table *tb, char *file, char *keyword)
{
...
void BondPolyFJC::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->r0 = 0.0;
...
void BondPolyFJC::bond_lookup(int type, int i, int j, double &N, double &b)
{
  int iatom, jatom;
  const Table *tb = &tables[tabindex[type]];
...
void BondPolyFJC::bcast_table(Table *tb) // *UPDATED
{
