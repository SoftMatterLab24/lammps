.. index:: pair_style poly/spring

pair_style poly/spring command
=============================
...
.. code-block:: LAMMPS
   ...
   pair_style poly/spring keyword value  ...
...
.. code-block:: LAMMPS
   ...
   pair_style poly/spring
   pair_coeff * * 1.0 1.0 1.0
   pair_style poly/spring
   pair_coeff * * 1.0 1.0 1.0 mol yes
   pair_style poly/spring anharmonic yes
   pair_coeff 1 1 1.0 1.0 1.0 50.0 mol yes
...
Style *poly/spring* computes pairwise forces with the formula
...
This pair style is designed for use in spring-based bonded particle
models and meso-scale polymer models.  It mirrors the construction of the :doc:`bpm/spring
<pairs_bpm_spring>` pair style, with an additional option to exclude pair interactions 
between atoms with the same molecule ID. 
...
If the *mol* keyword is set to *yes* in the pair_coeff command, 
then the pair interaction is only applied between atoms with different 
molecule IDs. Note that it is always expected as the last coefficient. 
...
This pair style is part of the POLY-NET package.  It is only enabled if
